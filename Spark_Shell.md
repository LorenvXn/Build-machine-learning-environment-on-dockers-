...
CassandraConnector(conf).withSessionDo { session => session.execute("CREATE TABLE IF NOT EXISTS streaming_key.streaming_cass1 (DATE TEXT PRIMARY KEY, LON  VARCHAR, LAT VARCHAR, BASE VARCHAR)")}


//store streams to Cassandra

 val lines = rawDstream.map(line => line._2.split(',')).map(s => (s(0).toDouble, s(1).toString,s(2).toDouble))
 
 lines.saveToCassandra("dev", "employee", SomeColumns("id", "name", "salary" ))
